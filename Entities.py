...
class Mailer(threading.Thread):
    ...
    def create_measurements(self):
        ...
        self.measurements["Loss Timestamp Counter"][
            current_clock] = self.get_counter_sum_of_timestamp_loss_msgs_from_agents()
        self.measurements["Message Sent Counter"][current_clock] = self.msg_sent_counter
        self.measurements["Message Received Counter"][
            current_clock] = self.get_counter_sum_msg_received_counter_from_agents()
    ...
    def place_single_msg_from_inbox_in_msgs_box(self, msg):
        ...
        ids_ = [ msg.sender, msg.receiver]
        ans = []
        for agent in self.agents_algorithm:
            if agent.simulation_entity.id_ in ids_:
                ans.append(agent)
        if len(ans)!=2:
            raise Exception()

        communication_disturbance_output = self.f_communication_disturbance(ans[0], ans[1])
...
class AgentArseni(AgentAlgorithm):
    def __init__(self, simulation_agent: AgentSimulation, value):
        ...
        for n_id in self.simulation_entity.neighbours_ids_list:
            self.local_view[n_id] = None
    ...
    def compute(self):
        domain_counter = {}
        for d in self.simulation_entity.domain:
            domain_counter[d] = 0

        for n, v in self.local_view.items():
            if v is not None:
                domain_counter[v] = domain_counter[v] + 1

        self.value = min(domain_counter, key=domain_counter.get)

    def get_list_of_msgs_to_send(self):
        sender = self.simulation_entity.id_
        information = self.value
        is_with_perfect_communication = False
        ans = []
        for n_id in self.simulation_entity.neighbours_ids_list:
            receiver = n_id
            m = Msg(sender=sender, receiver=receiver, information=information,
                    is_with_perfect_communication=is_with_perfect_communication)
            ans.append(m)
